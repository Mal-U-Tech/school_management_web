...
import { selectPassControlsArray } from 'src/app/store/pass-controls/pass-control.selectors';
import { takeWhile } from 'rxjs';
import { selectChosenScoresheet, selectScoresheetsArray } from 'src/app/store/scoresheet/scoresheet.selector';
...
export class ClassScoresheetComponent implements OnInit, OnDestroy {
  ...
  loadData() {
    ...
    this.passControls$.pipe(takeWhile(() => this.alive)).subscribe({
    ...
    this.selectedScoresheet$.pipe(takeWhile(() => this.alive)).subscribe({
      next: (data: IScoresheet) => {
        if(data){
          this.scoresheetName = data.name;
          this.scoresheetYear = data.year;
        }
      }
    })
            ...
            table: {
              ...
              body: [
                [
                  ...
                  this.addTableText(
                    this.scoresheetName + ' ' + this.scoresheetYear,
                    14,
                    '',
                    '0',
                    [2, 15],
